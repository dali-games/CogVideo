...
# Pre-load the pipelines at startup to avoid reloading for each request
pipelines = {}

@app.on_event("startup")
async def startup_event():
    global pipelines
    pipelines["t2v"] = CogVideoXPipeline.from_pretrained("THUDM/CogVideoX-5b", torch_dtype=torch.bfloat16)
    pipelines["i2v"] = CogVideoXImageToVideoPipeline.from_pretrained("THUDM/CogVideoX-5b-I2V", torch_dtype=torch.bfloat16)
    pipelines["v2v"] = CogVideoXVideoToVideoPipeline.from_pretrained("THUDM/CogVideoX-5b", torch_dtype=torch.bfloat16)
...
async def generate_video_endpoint(
    ...
    pipe = pipelines.get(generate_type)
    if pipe is None:
        return {"error": f"Unsupported generate_type: {generate_type}"}
    ...
    if generate_type == "i2v":
        if image_or_video is None:
            return {"error": "An image file must be provided for 'i2v' generation type."}
        # Read the uploaded image
        image_bytes = await image_or_video.read()
        image = Image.open(BytesIO(image_bytes)).convert("RGB")
    elif generate_type == "v2v":
        if image_or_video is None:
            return {"error": "A video file must be provided for 'v2v' generation type."}
